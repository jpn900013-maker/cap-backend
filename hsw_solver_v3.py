...
import re, jwt, asyncio, threading
...
async def _ensure_page(ua: str):
    ...
    if _page is None or _page.is_closed():
        context = await _browser.new_context(user_agent=ua)
...
async def _hsw_impl(req: str, site: str, sitekey: str, ua: str) -> str:
    global _current_hsw_url

    async with _lock:
        page = await _ensure_page(ua)
...
def hsw(req: str, site: str, sitekey: str, ua: str) -> str:
    _ensure_loop()
    future = asyncio.run_coroutine_threadsafe(_hsw_impl(req, site, sitekey, ua), _loop)
