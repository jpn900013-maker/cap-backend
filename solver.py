...
# Cross-platform tls_client compatibility:
# Windows wheel uses 'timeout_seconds', Linux wheel uses 'timeout'.
_tls_sig = inspect.signature(tls_client.Session.execute_request)
_TLS_TIMEOUT_KEY = 'timeout_seconds' if 'timeout_seconds' in _tls_sig.parameters else 'timeout'
def _tls_timeout(seconds=60):
    """Return the correct timeout kwarg dict for this platform's tls_client."""
    return {_TLS_TIMEOUT_KEY: seconds}
